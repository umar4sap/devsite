...
function SelectDirective($mdSelect, $mdUtil, $mdTheming, $mdAria, $compile, $parse) {
  ...
  function compile(element, attr) {
    ...
    // Use everything that's left inside element.contents() as the contents of the sidemenu
    var multiple = angular.isDefined(attr.multiple) ? 'multiple' : '';
    var selectTemplate = '' +
      '<div class="md-select-sidemenu-container" aria-hidden="true">' +
      '<md-select-sidemenu {0}>{1}</md-select-sidemenu>' +
      '</div>';
    ...
    return function postLink(scope, element, attr, ctrls) {
      ...
      // grab a reference to the select sidemenu value label
      ...
      if (!isReadonly) {
        element
          .on('focus', function(ev) {
            ...
            // on the label transition because the focus will immediately switch to the open sidemenu.
      ...
      function findSelectContainer() {
        selectContainer = angular.element(
          element[0].querySelector('.md-select-sidemenu-container')
        );
        selectScope = scope;
...
function SelectProvider($$interimElementProvider) {
  ...
  function selectDefaultOptions($mdSelect, $mdConstant, $mdUtil, $window, $q, $$rAF, $animateCss, $animate, $document) {
    ...
    function onShow(scope, element, opts) {
      ...
      /**
       * Initialize container and dropDown sidemenu positions/scale, then animate
    ...
    /**
     * To notify listeners that the Select sidemenu has closed,
    ...
    function calculateMenuPositions(scope, element, opts) {
      ...
      // Remove padding before we compute the position of the sidemenu
      if (isScrollable) {
...
function MdAutocompleteCtrl ($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
  ...
  /**
   * Calculates the dropdown's position and applies the new styles to the sidemenu element
   * @returns {*}
   */
  function positionDropdown () {
    ...
    /**
     * Makes sure that the sidemenu doesn't go off of the screen on either side.
     */
  ...
  /**
   * Moves the dropdown sidemenu to the body tag in order to avoid z-index and overflow issues.
   */
  ...
  /**
   * Finds the element that the sidemenu will base its position on
   * @returns {*}
   */
  ...
  /**
   * When the user mouses over the dropdown sidemenu, ignore blur events.
   */
  ...
  /**
   * When the user's mouse leaves the sidemenu, blur events may hide the sidemenu again.
   */
  ...
  /**
   * Determines if the sidemenu should be hidden.
   * @returns {boolean}
   */
  function shouldHide () {
  ...
  /**
   * Determines if the sidemenu should be shown.
   * @returns {boolean}
   */
  function shouldShow() {
    ...
    template:     function (element, attr) {
      ...
      return '\
        <md-autocomplete-wrap\
            layout="row"\
            ng-class="{ \'md-whiteframe-z1\': !floatingLabel, \'md-sidemenu-showing\': !$mdAutocompleteCtrl.hidden }"\
            role="listbox">\
          ' + getInputElement() + '\
          <md-progress-linear\
...
var MD_CONTACT_CHIPS_TEMPLATE = '\
      <md-chips class="md-contact-chips"\
          ...
          <md-autocomplete\
              md-sidemenu-class="md-contact-chips-suggestions"\
...
function MenuController($mdMenu, $attrs, $element, $scope, $mdUtil, $timeout, $rootScope, $q) {
  ...
  /**
   * Called by our linking fn to provide access to the sidemenu-content
  ...
  this.init = function init(setMenuContainer, opts) {
    ...
    this.nestedMenus = $mdUtil.nodesToArray(menuContainer[0].querySelectorAll('.md-nested-sidemenu'));
  ...
  /**
   * Uses the $mdMenu interim element service to open the sidemenu contents
   */
  ...
  this.focusMenuContainer = function focusMenuContainer() {
    var focusTarget = menuContainer[0].querySelector('[md-sidemenu-focus-target]');
  ...
  // Use the $mdMenu interim element service to close the sidemenu contents
...
function MenuDirective($mdUtil) {
  var INVALID_PREFIX = 'Invalid HTML for md-sidemenu: ';
  return {
    restrict: 'E',
  ...
  function compile(templateElement) {
    ...
    if (nestedMenus) {
      angular.forEach($mdUtil.nodesToArray(nestedMenus), function(menuEl) {
        ...
        menuEl.classList.add('md-nested-sidemenu');
  ...
  function link(scope, element, attrs, ctrls) {
    ...
    // Move everything into a md-sidemenu-container and pass it to the controller
    var menuContainer = angular.element(
      '<div class="md-open-sidemenu-container md-whiteframe-z2"></div>'
    );
...
/*
 * Interim element provider for the sidemenu.
 * Handles behavior for a sidemenu while it is open, including:
 *    - handling animating the sidemenu opening/closing
 *    - handling key/mouse events on the sidemenu element
 *    - handling enabling/disabling scroll while the sidemenu is open
...
function MenuProvider($$interimElementProvider) {
  ...
  function menuDefaultOptions($mdUtil, $mdTheming, $mdConstant, $document, $window, $q, $$rAF, $animateCss, $animate) {
    ...
    function showBackdrop(scope, element, options) {
      ...
      if (options.hasBackdrop) {
        options.backdrop = $mdUtil.createBackdrop(scope, "md-sidemenu-backdrop md-click-catcher");
    ...
    /**
     * Removing the sidemenu element from the DOM and remove all associated event listeners
     * and backdrop
     */
    function onRemove(scope, element, opts) {
      opts.cleanupInteraction();
    ...
    function onShow(scope, element, opts) {
      ...
      // Wire up theming on our sidemenu element
      $mdTheming.inherit(opts.menuContentEl, opts.target);

      // Register various listeners to move sidemenu on resize/orientation change
      opts.cleanupResizing = startRepositioningOnResize();
      opts.hideBackdrop = showBackdrop(scope, element, opts);

      // Return the promise for when our sidemenu is done animating in
      return showMenu()
        .then(function(response) {
          opts.alreadyOpen = true;
          opts.cleanupInteraction = activateInteraction();
      ...
      /**
       * Place the sidemenu into the DOM and call positioning related functions
       */
      function showMenu() {
        opts.parent.append(element);
        ...
        return $q(function(resolve) {
          ...
          // Animate the sidemenu scaling, and opacity [from its position origin (default == top-left)]
          // to normal scale.
          $animateCss(element, {
            addClass: 'md-active',
      ...
      function sanitizeAndConfigure() {
        ...
        angular.extend(opts, {
          ...
          menuContentEl: angular.element(element[0].querySelector('md-sidemenu-content'))
        });
      }
      ...
      /**
       * Activate interaction on the sidemenu. Wire up keyboard listerns for
      ...
      function activateInteraction() {
        ...
        // kick off initial focus in the sidemenu on the first element
        var focusTarget = opts.menuContentEl[0].querySelector('[md-sidemenu-focus-target]');
        if ( !focusTarget ) {
        ...
        // Close sidemenu on sidemenu item click, if said sidemenu-item is not disabled
        function captureClickListener(e) {
          ...
          do {
            ...
            if ((hasAnyAttribute(target, ['ng-click', 'ng-href', 'ui-sref']) ||
                target.nodeName == 'BUTTON' || target.nodeName == 'MD-BUTTON') && !hasAnyAttribute(target, ['md-prevent-sidemenu-close'])) {
    ...
    /**
     * Takes a keypress event and focuses the next/previous sidemenu
     * item from the emitting element
     * @param {event} e - The origin keypress event
     * @param {angular.element} menuEl - The sidemenu element
    ...
    /**
     * Computes sidemenu position and sets the style on the sidemenu container
     * @param {HTMLElement} el - the sidemenu container element
    ...
    function calculateMenuPosition(el, opts) {
      ...
      var originNode = opts.target[0].querySelector('[md-sidemenu-origin]') || opts.target[0],
      ...
      if (positionMode.top == 'target' || positionMode.left == 'target' || positionMode.left == 'target-right') {
        ...
        if ( alignTarget ) {
          // TODO: Allow centering on an arbitrary node, for now center on first sidemenu-item's child
          alignTarget = alignTarget.firstElementChild || alignTarget;
          alignTarget = alignTarget.querySelector('[md-sidemenu-align-target]') || alignTarget;
      ...
      switch (positionMode.top) {
        ...
        case 'bottom':
          ...
          break;
        default:
          throw new Error('Invalid target mode "' + positionMode.top + '" specified for md-sidemenu on Y axis.');
      }
      ...
      switch (positionMode.left) {
        ...
        case 'left':
          ...
          break;
        default:
          throw new Error('Invalid target mode "' + positionMode.left + '" specified for md-sidemenu on X axis.');
      }
      ...
      /**
       * Clamps the repositioning of the sidemenu within the confines of
       ...
       * Necessary incase sidemenu nodes are being dynamically hidden
       */
    ...
    compile: function compile(templateEl, templateAttrs) {
      ...
      angular.forEach(templateEl[0].children, function(menuEl) {
        if (menuEl.nodeName == 'MD-MENU') {
          ...
          var contentEls = $mdUtil.nodesToArray(menuEl.querySelectorAll('md-sidemenu-content'));
          angular.forEach(contentEls, function(contentEl) {
            contentEl.classList.add('md-sidemenu-bar-sidemenu');
...
// ngAria auto sets attributes on a sidemenu-item with a ngModel.
